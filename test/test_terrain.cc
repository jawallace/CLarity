...
TEST(terrain, construction)
{
    ...
    ASSERT_EQ(sz, t.size().first) << "Terrain reported incorrect size";
    ASSERT_EQ(sz, t.size().second) << "Terrain reported incorrect size";
    ASSERT_FLOAT_EQ(30.0, t.scale()) << "Terrain reported incorrect scale";

    for (auto r = 0; r < 512; r++) {
        for (auto c = 0; c < 512; c++) {
            ASSERT_FLOAT_EQ(0.0, t.at(r, c)) << "Terrain was not zeroed on construction";
        }
    }
}
...
TEST(terrain, at)
{
    ...
    t.at(100, 100) = 12.5;
    ASSERT_FLOAT_EQ(12.5, t.at(100, 100)) << "Could not set/get value with Terrain.at";
}
...
TEST(terrain, assignment_and_copy)
{
    ...
    ASSERT_FLOAT_EQ(12.5, t2.at(100, 100)) 
